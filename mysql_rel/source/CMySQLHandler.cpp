...
bool CMySQLHandler::FetchField(string column)
{
	...
	for(unsigned int i = 0;i < m_dwFields;i++) {
		//if(column.compare(tempField[i].name) == 0) {
...
{
	if(!m_bIsConnected) {
		Natives::getInstance()->Debug("CMySQLHandler::Seek() - You cannot call this function now. (Reason: Connection is dead)");
		return 0;
	}
	if(m_stResult == NULL) {
...
int CMySQLHandler::RetrieveRow()
{
	...
	if(!m_dwFields) m_dwFields = mysql_num_fields(m_stResult);
	if((m_stRow = mysql_fetch_row(m_stResult))) {
...
int CMySQLHandler::Ping()
{
	if(!m_bIsConnected) {
		...
		m_dwError = mysql_errno(m_stConnectionPtr);
		Natives::getInstance()->Debug("CMySQLHandler::Ping() - An error has occured (%s, Error ID: %d)", mysql_error(m_stConnectionPtr), m_dwError);
		return 1;
	} else {
...
{
	if(!m_bIsConnected) {
		Natives::getInstance()->Debug("CMySQLHandler::EscapeString() - You cannot call this function now. (Reason: Dead Connection)");
		return 0;
	}
