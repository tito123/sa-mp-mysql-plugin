...
cell AMX_NATIVE_CALL Natives::n_cache_get_row_int(AMX* amx, cell* params) {
	unsigned int cID = params[3] - 1;
	Mutex::getInstance()->_lockMutex();
	//Debug(">> cache_get_row_int( Connection handle: %d )", cID+1);
	VALID_CONNECTION_HANDLE("cache_get_row_int", cID);
	CMySQLHandler *cHandle = SQLHandle[cID];
	unsigned int a = params[1], b = params[2];
	int val = atoi(cHandle->m_sCache[a][b]);
	Mutex::getInstance()->_unlockMutex();
	return val;
}
...
cell AMX_NATIVE_CALL Natives::n_cache_get_row_float(AMX* amx, cell* params) {
	unsigned int cID = params[3] - 1;
	Mutex::getInstance()->_lockMutex();
	//Debug(">> cache_get_row_float( Connection handle: %d )", cID+1);
	VALID_CONNECTION_HANDLE("cache_get_row_float", cID);
	CMySQLHandler *cHandle = SQLHandle[cID];
	unsigned int a = params[1], b = params[2];
	float val = (float) atof(cHandle->m_sCache[a][b]);
	Mutex::getInstance()->_unlockMutex();
	return amx_ftoc(val);
}
...
cell AMX_NATIVE_CALL Natives::n_cache_get_field_int(AMX* amx, cell* params) {
	unsigned int cID = params[3] - 1;
	Mutex::getInstance()->_lockMutex();
	//Debug(">> cache_get_field_int( Field[] = %s )", szField);
	VALID_CONNECTION_HANDLE("cache_get_field_int", cID);
	CMySQLHandler *cHandle = SQLHandle[cID];
	unsigned int a = params[1];
	char* szField;
	AMX_H->GetCString(amx, params[2], szField);
...
cell AMX_NATIVE_CALL Natives::n_cache_get_field_float(AMX* amx, cell* params) {
	unsigned int cID = params[3] - 1;
	Mutex::getInstance()->_lockMutex();
	//Debug(">> cache_get_field_float( Field[] = %s )", szField);
	VALID_CONNECTION_HANDLE("cache_get_field_float", cID);
	CMySQLHandler *cHandle = SQLHandle[cID];
	unsigned int a = params[1];
	char* szField;
	AMX_H->GetCString(amx, params[2], szField);
	...
	if (a < cHandle->m_sCache.size() && cHandle->m_dwCacheFields > 0) {
		for (unsigned int i = 0; i < cHandle->m_dwCacheFields; i++) {
			if (!strcmp(szField, cHandle->m_szCacheFields[i])) {
				val = (float) atof(cHandle->m_sCache[a][i]); // TODO: Float conversion.
				break;
			}
		}
	}
...
void Natives::Debug(char *text, ...) {
	if (Debugging) {
		...
		FILE *file = fopen("mysql_log.txt", "a");
		fprintf(file, "[%s] %s\n", timeform, buffer);
		fclose(file);
		va_end(args);
	}
